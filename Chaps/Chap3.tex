...
而不是$\mathrm{He}^+ + \mathrm{H}$. 因为$\mathrm{He}$的电离能(24.6eV)比质子的电子亲和能(13.6eV)大. 或者说, $\mathrm{He}^+$的电子亲和能比$\mathrm{H}^+$的电子亲和能大. 所涉不同物种的精确能量列在表\ref{t3.4}中. 由于解离产物是闭壳层的, 我们可以预期, 限制性\hft 计算在大键长下的行为应当正确, 不会发生$\hd$中那种情况. 
\begin{table}[H]
	\centering
...
\end{align}
\begin{align}
\phi_2(\bfr)=&N[0.47449g_{1s}(0.453757,\bfr)+0.13424 g_{1s}(2.01330,\bfr)+0.001906 g_{1s}(13.3615,\bfr)] \notag\\
             =&0.817238g_{1s}(0.453757,\bfr)+0.13424 g_{1s}(2.01330,\bfr)+0.032828 g_{1s}(13.3615,\bfr)
\label{3.289}
\end{align}
...
因为极小基中用到的基函数很少，所以让这些基函数尽可能接近最优形式是很重要的。这立刻就排除了使用单个高斯函数的可能。我们可能倾向于使用Slater函数或者近似于原子轨道的已知形状的函数。极小基计算随着类似“Gaussian 70”这样的计算机程序的开发而进步，这些程序可以使用收缩的高斯函数来复现使用Slater轨道的极小基计算。STO-LG方法中每个基函数由L个素高斯函数收缩而来，会选择合适的收缩系数和收缩指数使得到的基函数接近Slater函数。我们在\ref{sec3.5.1}中已经讨论过1s的STO-3G基组。
...
STO-LG基组当然不是唯一可能的极小基。例如Steart等人就对每个Slater函数都进行收缩高斯函数的拟合，而不进行在一个壳层内收缩指数相同的约束。比起使用Slater函数或者拟合Slater函数，一个更合乎理性的选择是接近该原子的Hartree-Fock原子轨道的收缩基函数。然而计算表明，在极小基下，采用最优的指数的Slater函数比上述的Hartree-Fock原子轨道更优，分子中的轨道可能和组成该分子的原子的轨道相当不同。
\subsection{双Zeta基组：4-31G}
极小基的变化灵活性很受限，特别是当指数没有被优化的时候。改进极小基的第一步包括使用两个基函数来极小基中的一个基函数，即双zeta基组。相对于极小基中的优化轨道指数，这两个基函数的轨道指数的最佳值通常一个略高一点而另一个略低一点。这使得我们可以通过改变线性参数来扩张或者收缩基函数，而不是通过改变非线性的指数。SCF过程会根据分子环境是否需要有效轨道的扩张或者收缩来改变收缩轨道和扩张轨道的系数。另外，相对于STO-基组，双zeta基组多出了一个各向异性的自由度，因为，比如说p轨道在不同的方向可以有不同的大小。

4-31G基组并不完全是一个双zeta基组，因为只有价电子轨道的基函数翻倍，而内层轨道仍然每个只用一个基函数。它可以被叫做分裂价基组。内层轨道对于大多数的化学性质的贡献较小，而且分子间的差别不大。如果不将内层壳函数分开的话，总能量会受到一定影响，但偶极矩、价电离势、电荷密度、解离能以及大部分其他化学上感兴趣的计算量都影响不大。4-31G基组下，H和He有两个基函数，Li到Ne有9个基函数，Na到Ar有13个基函数，如此等等。对于氢，基函数的收缩如下：
\begin{align}
...
更靠外的氢的波函数没有收缩，而靠内的氢的波函数由三个基本的高斯函数收缩而来。除了数值上有所差异外，推导以上基函数的收缩系数和指数的过程与之前式\eqref{3.288}和式\eqref{3.289}中(4s)/[2s]的函数基本一致。
也就是说，4-31G基组不是通过对任何特定的函数形式拟合得到的，而是在选择了特定的收缩方式后，通过改变收缩系数和指数来最小化计算得到的原子的能量。4-31G的缩写表示价层基函数包括了三个素高斯函数的收缩（内函数）和一个素高斯函数（外函数），而内层电子的函数是四个素高斯函数的收缩。当然氢原子没有内层电子。

对于从Li到F的原子，收缩形式如下：
\begin{align}
	\phi_{1s}(\mathbf{r})=&\sum_{i=1}^4 d_{i,1s}g_{1s}(\alpha_{i,1s},\mathbf{r})\\
	\phi'_{2s}(\mathbf{r})=&\sum_{i=1}^3 d_{i,2s}g_{1s}(\alpha'_{i,2sp},\mathbf{r})\\
	\phi"_{2s}(\mathbf{r})=&g_{1s}(\alpha"_{2sp},\mathbf{r})\\
	\phi'_{2p}(\mathbf{r})=&\sum_{i=1}^3 d_{i,2p}g_{2p}(\alpha'_{i,2sp},\mathbf{r})\\
	\phi"_{2p}(\mathbf{r})=&g_{2p}(\alpha"_{2sp},\mathbf{r})
\end{align}

和在STO-3G基组中的情况相同，为了提高计算效率，2s和2p函数使用同一个收缩指数。给出上述函数形式后，收缩系数$d_1s,d'_{2s},d"_{2s},d'_{2p},d"_{2p}$以及收缩指数$\alpha_{1s},\alpha'_{2sp},\alpha"_{2sp}$显式地被改变，直到原子经SCF计算得到的能量达到最小值。4-31G基组在这方面和STO-3G基组不同，STO-3G基组是通过和已知的函数进行最小二乘拟合，或者使用一般的收缩策略，这种策略将之前未收缩的原子计算的结果进行收缩，而4-31G是选择了(3.300)到(3.304)的收缩形式，然后优化所有收缩参数。也就是说，这个基组是先收缩再优化，而非先优化再收缩得到的。使用一般的记号，4-31G收缩被写作(8s4p/4s)[3s2p/2s]。这个基组中包括了内层函数、内价函数、外价函数，分别由4、3、1个素函数收缩得到。

\begin{table}[H]
	\centering\caption{标准4-31G价层放缩因子}
	\begin{tabular}{ccc}
		\hline
		原子&$\zeta'$&$\zeta"$\\\hline
		H	 &1.20&1.15\\
...
由于这个基组是从原子计算中得到的，我们仍然需要针对分子环境来放缩指数。我们定义$\zeta'$为内价层的放缩指数，$\zeta"$为外价层的放缩指数，将相应的内价层和外价层的$\alpha$乘上相应指数的平方。只有价层会被放缩。表\ref{t3.9}给出了一组标准的4-31G的放缩系数。只有氢的放缩系数和1的差距比较大，而碳的外价层比内价层更加紧密。

\exercise{He原子的4-31G基组并没有被正式定义过，然而Huzinaga等人使用四个未收缩的1s高斯函数对He原子进行SCF计算，获得了He原子的1s归一化轨道的系数和优化指数，如下表所示。
...
请使用附录A中给出的重叠积分的形式推导4-31G He原子基组的收缩参数。
}
