...
也就是说，4-31G基组不是通过对任何特定的函数形式拟合得到的，而是在选择了特定的收缩方式后，通过改变收缩系数和指数来最小化计算得到的原子的能量。4-31G的缩写表示价层基函数包括了三个基础的高斯函数的收缩（内函数）和一个基础的高斯函数（外函数），而内层电子的函数是四个基础的高斯函数的收缩。当然氢原子没有内层电子。
